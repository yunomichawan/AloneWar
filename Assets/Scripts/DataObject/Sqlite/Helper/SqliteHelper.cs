...
namespace AloneWar.DataObject.Sqlite.Helper
{
    ...
    public class SqliteHelper : SingletonComponent<SqliteHelper>
    {
        ...
        private List<T> ExecuteQuery<T>(SqliteQueryBuilder sqliteQueryBuilder)
        {
            ...
            {
                // db接続(共通化すること)
                SqliteDatabase sqliteDatabase = this.CreateConenctInstance("");
                // sql実行
        ...
        private SqliteDatabase CreateConenctInstance(string dbName)
        {
    ...
    public class SqliteQueryBuilder
    {
        ...
        private void PropertyLoop(Type type,Action<PropertyInfo> callback)
        {
            PropertyInfo[] propertyInfoArray = type.GetProperties();
            foreach (PropertyInfo propertyInfo in propertyInfoArray)
            {
        ...
        public void CreateTableSql(Type tableType)
        {
            ...
            this.ExecutetSql.AppendLine(string.Format(CREATE_TABLE_SQL, tableName, string.Join(",", columnSql)));
        }

...
namespace AloneWar.DataObject.Sqlite.Helper
{
    ...
    public class SqliteDatabase
    {
        ...
        private static extern int sqlite3_exec(IntPtr hDb, string sql, IntPtr callback, IntPtr args, out IntPtr errorMessage);
        ...
        public DataTable ExecuteQueryToDataTable(string query)
        {
            ...
            this.ExecuteQuery(query, (stmHandle) =>
            {
        ...
        public List<T> ExecuteQueryToObject<T>(string query)
        {
        ...
        private void ExecuteQuery(string query, Action<IntPtr> callback)
        {
            if (!CanExQuery)
            {
                Debug.Log("ERROR: Can't execute the query, verify DB origin file");
        ...
        /// </summary>
        /// <param name="query"></param>
        public void ExecuteNonQueryEx(string query)
        {
            try
            {
                if (!CanExQuery)
